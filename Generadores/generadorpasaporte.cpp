#include "../Generadores/generadorpasaporte.h"
...
generar_pasaporte::generar_pasaporte(ReglasNivel1 * rules, AtributosComunes * atributos) {
    ...
    /*archivo.LeerArchivoNuevo("nacionalidades.txt");
    this->nacionalidades = archivo.getArray();
    this->max_nacionalidades = archivo.getTopeArray();*/
    this->rules = rules;
...
std::string generar_pasaporte::generar_fecha(bool valido){
    ...
    std::string fecha = std::to_string(generar_dia) + "/" +  std::to_string(generar_mes) + "/" +  std::to_string(generar_anio);
    return fecha;
}
//me llega si es valido, reglas, genero
std::string generar_pasaporte::generar_nacionalidad(bool valido){
    ...
    std::string nacionalidad_generada; // lo que se devuelve que es la nacionalidad, valida o no
...
std::string generar_pasaporte::generar_estado_civil(char genero, bool valido){
    ...
    std::string * estados_civiles = this->atributos->getEstadosCiviles(tamanio); // para conseguir estados civiles
    std::string * estados_civiles_validos = this->rules->getEstadoCivilPermitido(tamanio2);
    std::string estado_civil_generado;
...
std::string generar_pasaporte::generar_nombre(char genero){
    std::string nombre_generado; // string con el nombre que se va a pickear
...
Pasaporte * generar_pasaporte::crear_pasaporte(bool valido, char genero){
    ...
    std::string fecha_generada = generar_fecha(fecha_valida);
    std::string nacionalidad_generada = generar_nacionalidad(nacionalidad_valida);
    std::string estado_civil_generado = generar_estado_civil(genero, estado_civil_valido);
    std::string nombre_generado = generar_nombre( genero);
